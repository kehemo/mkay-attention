...
def flash_backward(Q, K, V, O, dO, l, m, softmax_scale):
    ...
    for j in range(Tc):
        ...
        Kj = K[kv_start:kv_end, :]
        Vj = V[kv_start:kv_end, :]
        dK_t_j = torch.zeros(Bc, d)
        dV_t_j = torch.zeros(Bc, d)
        ...
        for i in range(Tr):
            ...
            dQ[q_start:q_end, :] = dQ[q_start:q_end, :] + dSij @ Kj * softmax_scale
            dK_t_j = dK_t_j + dSij.T @ Qi * softmax_scale

        dK[kv_start:kv_end, :] = dK_t_j
        dV[kv_start:kv_end, :] = dV_t_j
